...
(function () {
    var w2grid = function(options) {
        ...
        this.records      = [];       // { builder: int(requied), field1: 'value1', ... fieldN: 'valueN', style: 'string', editable: true/false, summary: true/false, changes: object }
    ...
    $.fn.w2grid = function(method) {
        if (typeof method === 'object' || !method ) {
            ...
            // check if there are records without builder
            for (var r in records) {
                if (records[r].builder == null || typeof records[r].builder == 'undefined') {
                    console.log('ERROR: Cannot add records without builder. (obj: '+ object.builder +')');
                    return;
                }
            ...
            for (var c in object.columns) {
                ...
                var attr  = '';
                if (col.searchable === true) { stype = 'int'; attr = 'size="20"'; }
                else {
                    {stype='text';attr='size="20"';} 
                }
    ...
    w2grid.prototype = {
        ...
        add: function (record) {
            ...
            var added = 0;
            for (var o in record) {
                if (!this.builder && typeof record[o].builder == 'undefined') record[o].builder = record[o][this.builder];
                if (record[o].builder == null || typeof record[o].builder == 'undefined') {
                    console.log('ERROR: Cannot add record without builder. (obj: '+ this.name +')');
                    continue;
                }
        ...
        find: function (obj, returnIndex) {
            ...
            for (var i = 0; i < this.records.length; i++) {
                ...
                if (match && returnIndex !== true) recs.push(this.records[i].builder);
                if (match && returnIndex === true) recs.push(i);
            }
            return recs;
        },

        set: function (builder, record, noRefresh) { // does not delete existing, but overrides on top of it
            ...
            if (builder == null) {
                for (var r in this.records) {
                    $.extend(true, this.records[r], record); // builder is the whole record
                }
                if (noRefresh !== true) this.refresh();
            } else { // find record to update
                var ind = this.get(builder, true);
                if (ind == null) return false;
                var isSummary = (this.records[ind] && this.records[ind].builder == builder ? false : true);
                if (isSummary) {
                    $.extend(true, this.summary[ind], record);
                } else {
                    $.extend(true, this.records[ind], record);
                }
                if (noRefresh !== true) this.refreshRow(builder); // refresh only that record
            }
            return true;
        },

        get: function (builder, returnIndex) {
            // search records
            for (var i = 0; i < this.records.length; i++) {
                if (this.records[i].builder == builder) {
                    if (returnIndex === true) return i; else return this.records[i];
                }
            }
            // search summary
            for (var i = 0; i < this.summary.length; i++) {
                if (this.summary[i].builder == builder) {
        ...
        remove: function () {
            ...
            for (var a = 0; a < arguments.length; a++) {
                for (var r = this.records.length-1; r >= 0; r--) {
                    if (this.records[r].builder == arguments[a]) { this.records.splice(r, 1); removed++; }
                }
            }
            var url = (typeof this.url != 'object' ? this.url : this.url.get);
            if (!url) {
                    ...
                    for (var s in this.searchData) {
                        ...
                        switch (sdata.operator) {
                            case 'is':
                                ...
                                break;
                            case 'between':
                                if (['int', 'float', 'money', 'currency', 'percent','number'].indexOf(search.type) != -1) {
        ...
        getRangeData: function (range, extra) {
            var rec1 = this.get(range[0].builder, true);
            var rec2 = this.get(range[1].builder, true);
            ...
            for (var r in ranges) {
                ...
                if (ranges[r].name == 'selection') {
                    ...
                    if (sel.length == 0) {
                        ...
                        var td1   = $('#grid_'+ this.name +'_rec_'+ first.builder + ' td[col='+ first.column +']');
                        var td2   = $('#grid_'+ this.name +'_rec_'+ last.builder + ' td[col='+ last.column +']');
                    }
                } else { // other range
                ...
                if (first) {
                    var rg = {
                        name: ranges[r].name,
                        range: [{ builder: first.builder, column: first.column }, { builder: last.builder, column: last.column }],
        ...
        refreshRanges: function () {
            ...
            function mouseStart (event) {
                ...
                obj.last.move = {
                    ...
                    divX   : 0,
                    divY   : 0,
                    builder  : sel[0].builder,
                    column : sel[0].column,
                    originalRange : [{ builder: sel[0].builder, column: sel[0].column }, { builder: sel[sel.length-1].builder, column: sel[sel.length-1].column }],
                    newRange      : [{ builder: sel[0].builder, column: sel[0].column }, { builder: sel[sel.length-1].builder, column: sel[sel.length-1].column }]
                };
            ...
            function mouseMove (event) {
                ...
                builder = $(tmp).attr('builder');
                // new range
                if (mv.newRange[1].builder == builder && mv.newRange[1].column == column) return;
                var prevNewRange = $.extend({}, mv.newRange);
                mv.newRange = [{ builder: mv.builder, column: mv.column }, { builder: builder, column: column }];
                // event before
        ...
        select: function () {
            ...
            for (var a = 0; a < arguments.length; a++) {
                var builder  = typeof arguments[a] == 'object' ? arguments[a].builder : arguments[a];
                var record = this.get(builder);
                if (record == null) continue;
                var index  = this.get(builder, true);
                var recEl  = $('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(builder));
                if (this.selectType == 'row') {
                    if (sel.indexes.indexOf(index) >= 0) continue;
                    // event before
                    var eventData = this.trigger({ phase: 'before', type: 'select', target: this.name, builder: builder, index: index });
                    ...
                    if (!w2utils.isInt(col)) { // select all columns
                        var cols = [];
                        for (var c in this.columns) { if (this.columns[c].hidden) continue; cols.push({ builder: builder, column: parseInt(c) }); }
                    ...
                    // event before
                    var eventData = this.trigger({ phase: 'before', type: 'select', target: this.name, builder: builder, index: index, column: col });
                    if (eventData.isCancelled === true) continue;
                    // default action
        ...
        unselect: function () {
            ...
            for (var a = 0; a < arguments.length; a++) {
                ...
                var record = this.get(builder);
                if (record == null) continue;
                var index  = this.get(record.builder, true);
                var recEl  = $('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(builder));
                if (this.selectType == 'row') {
                    if (sel.indexes.indexOf(index) == -1) continue;
                    // event before
                    var eventData = this.trigger({ phase: 'before', type: 'unselect', target: this.name, builder: builder, index: index });
                    if (eventData.isCancelled === true) continue;
                    // default action
                    ...
                    if (!$.isArray(s) || s.indexOf(col) == -1) continue;
                    // event before
                    var eventData = this.trigger({ phase: 'before', type: 'unselect', target: this.name, builder: builder, column: col });
                    if (eventData.isCancelled === true) continue;
                    // default action
                    s.splice(s.indexOf(col), 1);
                    $('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(builder) + ' > td[col='+ col +']').removeClass('w2ui-selected');
                    unselected++;
                    if (s.length == 0) {
        ...
        selectNone: function () {
            ...
            for (var s in sel.indexes) {
                ...
                var builder = rec ? rec.builder : null;
                var recEl = $('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(builder));
                recEl.removeClass('w2ui-selected').removeData('selected');
                recEl.find('.w2ui-grid-select-check').prop("checked", false);
                // for not rows
        ...
        getSelection: function (returnIndex) {
            ...
            if (this.selectType == 'row') {
                for (var s in sel.indexes) {
                    ...
                    if (returnIndex === true) ret.push(sel.indexes[s]); else ret.push(this.records[sel.indexes[s]].builder);
                }
                return ret;
            } else {
                for (var s in sel.indexes) {
                    ...
                    if (!this.records[sel.indexes[s]]) continue;
                    for (var c in cols) {
                        ret.push({ builder: this.records[sel.indexes[s]].builder, index: parseInt(sel.indexes[s]), column: cols[c] });
                    }
                }
                return ret;
            }
                    ...
                    if (field.toLowerCase() == 'all') {
                        ...
                        if (this.searches.length > 0) {
                            for (var s in this.searches) {
                                ...
                                if (['int', 'float', 'money', 'currency', 'percent','number'].indexOf(search.type) != -1 && String(value).indexOf('-') != -1) {
                ...
                .fail(function (xhr, status, error) {
                    ...
                    if (status != 'abort') {
                        ...
                        console.log('ERROR: Server communication failed.', 
                            '\n   EXPECTED:', { status: 'success', total: 5, records: [{ builder: 1, field: 'value' }] }, 
                ...
                if (typeof responseText != 'undefined' && responseText != '') {
                    ...
                    if (obj.builder) {
                        for (var r in data.records) {
                            data.records[r]['builder'] = data.records[r][obj.builder];
                        }
                    }
        ...
        getChanges: function () {
            ...
            for (var r in this.records) {
                ...
                if (typeof rec['changes'] != 'undefined') {
                    changes.push($.extend(true, { builder: rec.builder }, rec.changes));
                }
            }
            return changes;
        },
        ...
        mergeChanges: function () {
            ...
            for (var c in changes) {
                var record = this.get(changes[c].builder);
                for (var s in changes[c]) {
                    if (s == 'builder') continue; // do not allow to change builder
        ...
        editField: function (builder, column, value, event) {
            var obj   = this;
            var index = obj.get(builder, true);
            ...
            // event before
            var eventData = obj.trigger({ phase: 'before', type: 'editField', target: obj.name, builder: builder, column: column, value: value,
                index: index, originalEvent: event });
            if (eventData.isCancelled === true) return;
            value = eventData.value;
            // default behaviour
            this.selectNone();
            this.select({ builder: builder, column: column });
            this.last.edit_col = column;
            if (['checkbox', 'check'].indexOf(edit.type) != -1) return;
            // create input element
            var tr = $('#grid_'+ obj.name +'_rec_'+ w2utils.escapeId(builder));
            ...
            if (edit.type == 'select') {
                ...
                el.addClass('w2ui-editable')
                    .html('<select id="grid_'+ obj.name +'_edit_'+ builder +'_'+ column +'" column="'+ column +'" '+
                        '    style="width: 100%; '+ addStyle + edit.style +'" field="'+ col.field +'" builder="'+ builder +'" '+
                        '    '+ edit.inTag +
                ...
                el.addClass('w2ui-editable')
                    .html('<input id="grid_'+ obj.name +'_edit_'+ builder +'_'+ column +'" '+
                        '    type="text" style="font-family: inherit; font-size: inherit; outline: none; '+ addStyle + edit.style +'" field="'+ col.field +'" builder="'+ builder +'" '+
                    ...
                    .on('keydown', function (event) {
                        ...
                        switch (event.keyCode) {
                            case 9:  // tab
                                ...
                                if (next_col == null) {
                                    ...
                                    if (tmp != null && tmp != index) {
                                        next_rec = obj.records[tmp].builder;
                                ...
                                if (next_rec === false) next_rec = builder;
                                ...
                                setTimeout(function () {
                                    if (obj.selectType != 'row') {
                                        obj.selectNone();
                                        obj.select({ builder: next_rec, column: next_col });
                                    } else {
                            ...
                            case 13: // enter
                                ...
                                if (next != null && next != index) {
                                    setTimeout(function () {
                                        if (obj.selectType != 'row') {
                                            obj.selectNone();
                                            obj.select({ builder: obj.records[next].builder, column: column });
                                        } else {
                                            obj.editField(obj.records[next].builder, column, null, event);
                                        }
                                    }, 100);
                                }
                                break;
                            ...
                            case 27: // escape
                                ...
                                this.blur();
                                setTimeout(function () { obj.select({ builder: builder, column: column }) }, 1);
                                break;
                        }
        ...
        editChange: function (el, index, column, event) {
            ...
            var tr      = $('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(rec.builder));
            ...
            var eventData = {
                phase: 'before', type: 'change', target: this.name, input_id: el.id, builder: rec.builder, index: index, column: column,
            ...
            if (url) {
                ...
                if (typeof recs[0] != 'object') {
                    ...
                    for (var r in recs) {
                        ...
                        var ind = this.get(recs[r].builder, true);
                        if (ind != null && fld != 'builder') {
        ...
        click: function (builder, event) {
            ...
            // event before
            var eventData = this.trigger({ phase: 'before', target: this.name, type: 'click', builder: builder, column: column, originalEvent: event });
            if (eventData.isCancelled === true) return;
            // if it is subgrid unselect top grid
            var parent = $('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(builder)).parents('tr');
            ...
            if (event.shiftKey && sel.length > 0 && obj.multiSelect) {
                ...
                for (var i = start; i <= end; i++) {
                    ...
                    if (this.selectType == 'row') {
                        sel_add.push(this.records[i].builder);
                    } else {
                        for (var sc in selectColumns) sel_add.push({ builder: this.records[i].builder, column: selectColumns[sc] });
                    }
                    //sel.push(this.records[i].builder);
                }
                ...
                if (((!event.ctrlKey && !event.shiftKey && !event.metaKey) || !this.multiSelect) && !this.showSelectColumn) {
                    ...
                    if (flag === true) {
                        this.unselect({ builder: builder, column: column });
                    } else {
                        this.select({ builder: builder, column: column });
                    }
                } else {
        ...
        keydown: function (event) {
            ...
            var builder   = sel[0] || null;
            var columns = [];
            var builder2  = sel[sel.length-1];
            if (typeof builder == 'object' && builder != null) {
                builder   = sel[0].builder;
                columns = [];
                var ii  = 0;
                while (true) {
                    if (!sel[ii] || sel[ii].builder != builder) break;
                    columns.push(sel[ii].column);
                    ii++;
                }
                builder2  = sel[sel.length-1].builder;
            }
            var ind    = obj.get(builder, true);
            var ind2   = obj.get(builder2, true);
            var rec    = obj.get(builder);
            var recEL  = $('#grid_'+ obj.name +'_rec_'+ (ind !== null ? w2utils.escapeId(obj.records[ind].builder) : 'none'));
            var cancel = false;
            var key    = event.keyCode;
            ...
            switch (key) {
                ...
                case 27: // escape
                    ...
                    if (sel.length > 0 && typeof sel[0] == 'object') {
                        obj.select({ builder: sel[0].builder, column: sel[0].column });
                    }
                    cancel = true;
                    break;
                ...
                case 13: // enter
                    ...
                    if (this.selectType == 'row' && obj.show.expandColumn === true) {
                        ...
                        if (columns.length > 0) {
                            obj.editField(builder, columns[0], null, event);
                            cancel = true;
                        }
                    }
                    break;
                ...
                case 37: // left
                    ...
                    var parent = $('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(obj.records[ind].builder)).parents('tr');
                    if (parent.length > 0 && String(parent.attr('id')).indexOf('expanded_row') != -1) {
                        var builder = parent.prev().attr('builder');
                        var grid  = parent.parents('.w2ui-grid').attr('name');
                        obj.selectNone();
                        w2utils.keyboard.active(grid);
                        w2ui[grid].set(builder, { expanded: false });
                        w2ui[grid].collapse(builder);
                        w2ui[grid].click(builder);
                        cancel = true;
                        break;
                    }
                    if (this.selectType == 'row') {
                        if (recEL.length <= 0 || rec.expanded !== true ) break;
                        obj.set(builder, { expanded: false }, true);
                        obj.collapse(builder, event);
                    } else {
                        ...
                        if (prev != null) {
                            if (shiftKey && obj.multiSelect) {
                                ...
                                if (columns.indexOf(this.last.sel_col) == 0 && columns.length > 1) {
                                    for (var i in sel) {
                                        if (tmp.indexOf(sel[i].builder) == -1) tmp.push(sel[i].builder);
                                        unSel.push({ builder: sel[i].builder, column: columns[columns.length-1] });
                                    }
                                } else {
                                    for (var i in sel) {
                                        if (tmp.indexOf(sel[i].builder) == -1) tmp.push(sel[i].builder);
                                        newSel.push({ builder: sel[i].builder, column: prev });
                                    }
                                }
                                ...
                                obj.click({ builder: builder, column: prev }, event);
                            }
                        } else {
                        ...
                        if (next !== null) {
                            if (shiftKey && key == 39 && obj.multiSelect) {
                                ...
                                if (columns.indexOf(this.last.sel_col) == columns.length-1 && columns.length > 1) {
                                    for (var i in sel) {
                                        if (tmp.indexOf(sel[i].builder) == -1) tmp.push(sel[i].builder);
                                        unSel.push({ builder: sel[i].builder, column: columns[0] });
                                    }
                                } else {
                                    for (var i in sel) {
                                        if (tmp.indexOf(sel[i].builder) == -1) tmp.push(sel[i].builder);
                                        newSel.push({ builder: sel[i].builder, column: next });
                                    }
                                }
                                obj.unselect.apply(obj, unSel);
                                obj.select.apply(obj, newSel);
                            } else {
                                obj.click({ builder: builder, column: next }, event);
                            }
                        } else {
                            // if selected more then one, then select first
                            if (!shiftKey) {
                ...
                case 38: // up
                    ...
                    if (prev != null) {
                        ...
                        if (obj.records[prev].expanded) {
                            var subgrid = $('#grid_'+ obj.name +'_rec_'+ w2utils.escapeId(obj.records[prev].builder) +'_expanded_row').find('.w2ui-grid');
                            if (subgrid.length > 0 && w2ui[subgrid.attr('name')]) {
                                obj.selectNone();
                                var grid = subgrid.attr('name');
                                var recs = w2ui[grid].records;
                                w2utils.keyboard.active(grid);
                                w2ui[grid].click(recs[recs.length-1].builder);
                                cancel = true;
                                break;
                            }
                        }
                        if (shiftKey && obj.multiSelect) { // expand selection
                            ...
                            if (obj.selectType == 'row') {
                                if (obj.last.sel_ind > prev && obj.last.sel_ind != ind2) {
                                    obj.unselect(obj.records[ind2].builder);
                                } else {
                                    obj.select(obj.records[prev].builder);
                                }
                            } else {
                                if (obj.last.sel_ind > prev && obj.last.sel_ind != ind2) {
                                    prev = ind2;
                                    var tmp = [];
                                    for (var c in columns) tmp.push({ builder: obj.records[prev].builder, column: columns[c] });
                            ...
                            obj.selectNone();
                            obj.click({ builder: obj.records[prev].builder, column: columns[0] }, event);
                        }
                        ...
                        var parent = $('#grid_'+ obj.name +'_rec_'+ w2utils.escapeId(obj.records[ind].builder)).parents('tr');
                        if (parent.length > 0 && String(parent.attr('id')).indexOf('expanded_row') != -1) {
                            var builder = parent.prev().attr('builder');
                            var grid  = parent.parents('.w2ui-grid').attr('name');
                            obj.selectNone();
                ...
                case 40: // down
                    ...
                    if (obj.records[ind2].expanded) {
                        var subgrid = $('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(obj.records[ind2].builder) +'_expanded_row').find('.w2ui-grid');
                        if (subgrid.length > 0 && w2ui[subgrid.attr('name')]) {
                            obj.selectNone();
                            var grid = subgrid.attr('name');
                            var recs = w2ui[grid].records;
                            w2utils.keyboard.active(grid);
                            w2ui[grid].click(recs[0].builder);
                            cancel = true;
                            break;
                        }
                    }
                    ...
                    if (next != null) {
                        if (shiftKey && obj.multiSelect) { // expand selection
                            ...
                            if (obj.selectType == 'row') {
                                if (this.last.sel_ind < next && this.last.sel_ind != ind) {
                                    obj.unselect(obj.records[ind].builder);
                                } else {
                                    obj.select(obj.records[next].builder);
                                }
                            } else {
                                if (this.last.sel_ind < next && this.last.sel_ind != ind) {
                                    next = ind;
                                    var tmp = [];
                                    for (var c in columns) tmp.push({ builder: obj.records[next].builder, column: columns[c] });
                                    obj.unselect.apply(obj, tmp);
                                } else {
                                    var tmp = [];
                                    for (var c in columns) tmp.push({ builder: obj.records[next].builder, column: columns[c] });
                            ...
                            obj.selectNone();
                            obj.click({ builder: obj.records[next].builder, column: columns[0] }, event);
                        }
                        ...
                        var parent = $('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(obj.records[ind2].builder)).parents('tr');
                        if (parent.length > 0 && String(parent.attr('id')).indexOf('expanded_row') != -1) {
                            var builder = parent.next().attr('builder');
                            var grid  = parent.parents('.w2ui-grid').attr('name');
                            obj.selectNone();
                            w2utils.keyboard.active(grid);
                            w2ui[grid].click(builder);
            ...
            if (tmp.indexOf(key) != -1 && !event.ctrlKey && !event.metaKey && !cancel) {
                ...
                obj.editField(builder, columns[0], tmp, event);
                cancel = true;
            }
            ...
            function selectTopRecord() {
                ...
                obj.select({ builder: obj.records[ind].builder, column: 0});
            }
            ...
            function tmpUnselect () {
                ...
                if (obj.selectType != 'row') {
                    ...
                    if (sel.length > 1) {
                        for (var s in sel) {
                            if (sel[s].builder == obj.last.sel_builder && sel[s].column == obj.last.sel_col) {
                                sel.splice(s, 1);
                                break;
                            }
                        }
                    ...
                    obj.last.sel_type = 'key';
                    if (sel.length > 1) {
                        sel.splice(sel.indexOf(obj.records[obj.last.sel_ind].builder), 1);
                        obj.unselect.apply(obj, sel);
                        return true;
                    }
                    return false;
        ...
        dblClick: function (builder, event) {
            ...
            // event before
            var eventData = this.trigger({ phase: 'before', target: this.name, type: 'dblClick', builder: builder, column: column, originalEvent: event });
            if (eventData.isCancelled === true) return;
            // default action
            this.selectNone();
            var col = this.columns[column];
            if (col && $.isPlainObject(col.editable)) {
                this.editField(builder, column, null, event);
            } else {
                this.select({ builder: builder, column: column });
            }
            // event after
        ...
        contextMenu: function (builder, event) {
            var obj = this;
            if (obj.last.userSelect == 'text') return;
            if (typeof event == 'undefined') event = { offsetX: 0, offsetY: 0, target: $('#grid_'+ obj.name +'_rec_'+ builder)[0] };
            if (typeof event.offsetX === 'undefined') {
                event.offsetX = event.layerX - event.target.offsetLeft;
                event.offsetY = event.layerY - event.target.offsetTop;
            }
            if (w2utils.isFloat(builder)) builder = parseFloat(builder);
            if (this.getSelection().indexOf(builder) == -1) obj.click(builder);
            // need timeout to allow click to finish first
            setTimeout(function () {
                // event before
                var eventData = obj.trigger({ phase: 'before', type: 'contextMenu', target: obj.name, originalEvent: event, builder: builder });
                if (eventData.isCancelled === true) return;
                // default action
                    ...
                    $(obj.box).find(event.target)
                        .w2menu(obj.menu, {
                            ...
                            onSelect: function (event) { 
                                obj.menuClick(builder, parseInt(event.index), event.originalEvent); 
                            }
                        }
                    );
                }
        ...
        menuClick: function (builder, index, event) {
            var obj = this;
            // event before
            var eventData = obj.trigger({ phase: 'before', type: 'menuClick', target: obj.name, originalEvent: event, 
                builder: builder, menuIndex: index, menuItem: obj.menu[index] });
            if (eventData.isCancelled === true) return;
            // default action
            // -- empty
            // event after
        ...
        toggle: function (builder) {
            var rec = this.get(builder);
            if (rec.expanded === true) return this.collapse(builder); else return this.expand(builder);
        },

        expand: function (builder) {
            var rec = this.get(builder);
            var obj = this;
            var id  = w2utils.escapeId(builder);
            ...
            var addClass = ''; // ($('#grid_'+this.name +'_rec_'+ w2utils.escapeId(builder)).hasClass('w2ui-odd') ? 'w2ui-odd' : 'w2ui-even');
            $('#grid_'+ this.name +'_rec_'+ id).after(
                    '<tr id="grid_'+ this.name +'_rec_'+ builder +'_expanded_row" class="w2ui-expanded-row '+ addClass +'">'+
                        (this.show.lineNumbers ? '<td class="w2ui-col-number"></td>' : '') +
                    '    <td class="w2ui-grid-data w2ui-expanded1" colspan="'+ tmp +'"><div style="display: none"></div></td>'+
                    '    <td colspan="100" class="w2ui-expanded2">'+
                    '        <div id="grid_'+ this.name +'_rec_'+ builder +'_expanded" style="opacity: 0"></div>'+
                    '    </td>'+
                    '</tr>');
            // event before
            var eventData = this.trigger({ phase: 'before', type: 'expand', target: this.name, builder: builder,
                box_id: 'grid_'+ this.name +'_rec_'+ builder +'_expanded', ready: ready });
            if (eventData.isCancelled === true) {
            ...
            $('#grid_'+ this.name +'_cell_'+ this.get(builder, true) +'_expand div').html('<div class="w2ui-spinner" style="width: 16px; height: 16px; margin: -2px 2px;"></div>');
            rec.expanded = true;
            ...
            function ready() {
                ...
                $('#grid_'+ obj.name +'_cell_'+ obj.get(builder, true) +'_expand div').html('-');
            }
            // event after
            this.trigger($.extend(eventData, { phase: 'after' }));
            this.resizeRecords();
        ...
        collapse: function (builder) {
            var rec = this.get(builder);
            var obj = this;
            var id  = w2utils.escapeId(builder);
            if ($('#grid_'+ this.name +'_rec_'+ id +'_expanded_row').length == 0) return false;
            // event before
            var eventData = this.trigger({ phase: 'before', type: 'collapse', target: this.name, builder: builder,
            ...
            $('#grid_'+ this.name +'_cell_'+ this.get(builder, true) +'_expand div').html('+');
            setTimeout(function () {
        ...
        paste: function (text) {
            var sel = this.getSelection();
            var ind = this.get(sel[0].builder, true);
            ...
            for (var t in text) {
                ...
                for (var c in cols) newSel.push({ builder: rec.builder, column: cols[c] });
                ind++;
            }
            this.selectNone();
        ...
        refreshCell: function (builder, field) {
            var index     = this.get(builder, true);
            var isSummary = (this.records[index] && this.records[index].builder == builder ? false : true);
            var col_ind   = this.getColumn(field, true);
            var rec       = (isSummary ? this.summary[index] : this.records[index]);
            var col       = this.columns[col_ind];
            var cell      = $('#grid_'+ this.name + '_rec_'+ builder +' [col='+ col_ind +']');
        ...
        refreshRow: function (builder) {
            var tr = $('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(builder));
            if (tr.length != 0) {
                var ind  = this.get(builder, true);
        ...
        render: function (box) {
            ...
            function mouseStart (event) {
                ...
                if (event.altKey) {
                    ...
                    obj.last.move = {
                        ...
                        divX   : 0,
                        divY   : 0,
                        builder  : $(event.target).parents('tr').attr('builder'),
            ...
            function mouseMove (event) {
                ...
                if (obj.reorderRows == true) {
                    if (!mv.ghost) {
                        var row    = $('#grid_'+ obj.name + '_rec_'+ mv.builder);
                    ...
                    var builder = $(event.target).parents('tr').attr('builder');
                    if (builder != mv.from) {
                        var row1 = $('#grid_'+ obj.name + '_rec_'+ mv.builder);
                        var row2 = $('#grid_'+ obj.name + '_rec_'+ builder);
                ...
                if (mv.start && mv.builder) {
                    obj.selectNone();
                    mv.start = false;
                }
                var newSel= [];
                var builder = (event.target.tagName == 'TR' ? $(event.target).attr('builder') : $(event.target).parents('tr').attr('builder'));
                if (typeof builder == 'undefined') return;
                var ind1  = obj.get(mv.builder, true);
                // |:wolfmanx:| this happens when selection is started on summary row
                if (ind1 === null) return;
                var ind2  = obj.get(builder, true);
                ...
                for (var i = ind1; i <= ind2; i++) {
                    ...
                    if (obj.selectType != 'row') {
                        ...
                        for (var c = col1; c <= col2; c++) {
                            ...
                            newSel.push({ builder: obj.records[i].builder, column: parseInt(c) });
                        }
                    } else {
                        newSel.push(obj.records[i].builder);
                    }
                }
                if (obj.selectType != 'row') {
                    var sel = obj.getSelection();
                    ...
                    for (var ns in newSel) {
                        var flag = false;
                        for (var s in sel) if (newSel[ns].builder == sel[s].builder && newSel[ns].column == sel[s].column) flag = true;
                        if (!flag) tmp.push({ builder: newSel[ns].builder, column: newSel[ns].column });
                    }
                    obj.select.apply(obj, tmp);
                    // remove items
                    var tmp = [];
                    for (var s in sel) {
                        var flag = false;
                        for (var ns in newSel) if (newSel[ns].builder == sel[s].builder && newSel[ns].column == sel[s].column) flag = true;
                        if (!flag) tmp.push({ builder: sel[s].builder, column: sel[s].column });
                    }
                    obj.unselect.apply(obj, tmp);
                } else {
                    if (obj.multiSelect) {
            ...
            if (typeof this.toolbar['render'] == 'undefined') {
                ...
                this.toolbar.on('click', function (event) {
                    ...
                    switch (id) {
                        ...
                        case 'w2ui-add':
                            // events
                            var eventData = obj.trigger({ phase: 'before', target: obj.name, type: 'add', builder: null });
                            obj.trigger($.extend(eventData, { phase: 'after' }));
                            break;
                        case 'w2ui-edit':
                            var sel     = obj.getSelection();
                            var builder     = null;
                            if (sel.length == 1) builder = sel[0];
                            // events
                            var eventData = obj.trigger({ phase: 'before', target: obj.name, type: 'edit', builder: builder });
                            obj.trigger($.extend(eventData, { phase: 'after' }));
                            break;
                        case 'w2ui-delete':
                            obj["delete"]();
            ...
            for (var i = 0; i < this.searches.length; i++) {
                ...
                if (['text', 'alphanumeric', 'combo'].indexOf(s.type) != -1) {
                    ...
                    var operator =  '<select id="grid_'+ this.name +'_operator_'+ i +'" onclick="event.stopPropagation();">'+
                        '    <option value="is" selected="selected">'+ w2utils.lang('is') +'</option>'+
                        '    <option value="contains">'+ w2utils.lang('contains') +'</option>'+
                        '</select>';
                }
                if (['int', 'float', 'money', 'currency', 'percent', 'date', 'time','number'].indexOf(s.type) != -1) {
        ...
        getRecordHTML: function (ind, lineNum, summary) {
            ...
            if (!record) return '';
            var id = w2utils.escapeId(record.builder);
            var isRowSelected = false;
            if (sel.indexes.indexOf(ind) != -1) isRowSelected = true;
            // render TR
            rec_html += '<tr id="grid_'+ this.name +'_rec_'+ record.builder +'" builder="'+ record.builder +'" line="'+ lineNum +'" '+
                ' class="'+ (lineNum % 2 == 0 ? 'w2ui-even' : 'w2ui-odd') + (isRowSelected && this.selectType == 'row' ? ' w2ui-selected' : '') + (record.expanded === true ? ' w2ui-expanded' : '') + '" ' +
                (summary !== true ?
                    (w2utils.isIOS ?
                        '    onclick  = "w2ui[\''+ this.name +'\'].dblClick(\''+ record.builder +'\', event);"'
                        :
                        '    onclick  = "w2ui[\''+ this.name +'\'].click(\''+ record.builder +'\', event);"'+
                        '    oncontextmenu = "w2ui[\''+ this.name +'\'].contextMenu(\''+ record.builder +'\', event);"'
                     )
                    : ''
                ) +
            ...
            if (this.show.selectColumn) {
                rec_html +=
                        ...
                        '        onclick="if (event.stopPropagation) event.stopPropagation(); else event.cancelBubble = true;">'+
                            ...
                            '                if (this.checked) obj.select(\''+ record.builder + '\'); else obj.unselect(\''+ record.builder + '\'); '+
                            '                if (event.stopPropagation) event.stopPropagation(); else event.cancelBubble = true;">'+
                            '    </div>'
                            :
                            '' ) +
            ...
            if (this.show.expandColumn) {
                ...
                rec_html +=
                        '<td id="grid_'+ this.name +'_cell_'+ ind +'_expand' + (summary ? '_s' : '') + '" class="w2ui-grid-data w2ui-col-expand">'+
                            ...
                            '            onclick="w2ui[\''+ this.name +'\'].toggle(\''+ record.builder +'\', event); '+
                            '                if (event.stopPropagation) event.stopPropagation(); else event.cancelBubble = true;">'+
                            '        '+ tmp_img +' </div>'
                            :
                            '' ) +
        ...
        status: function (msg) {
            if (typeof msg != 'undefined') {
                ...
                if (sel.length > 0) {
                    ...
                    var tmp = sel[0];
                    if (typeof tmp == 'object') tmp = tmp.builder + ', '+ w2utils.lang('Column') +': '+ tmp.column;
    ...
    */
    ...
    w2field.prototype = {
        ...
        change: function (event) {
            ...
            // numeric
            if (['int', 'float', 'money', 'currency', 'percent','number'].indexOf(this.type) != -1) {
                // check max/min
                var val     =  $(this.el).val();
        ...
        blur: function (event) {
            ...
            if (['int', 'float', 'money', 'currency', 'percent','number'].indexOf(obj.type) != -1) {
        ...
        keyPress: function (event) {
            ...
            if (['int', 'float', 'money', 'currency','number', 'percent', 'hex', 'color', 'alphanumeric'].indexOf(obj.type) != -1) {
        ...
        keyDown: function (event, extra) {
            ...
            // numeric
            if (['int', 'float', 'money', 'currency','number', 'percent'].indexOf(obj.type) != -1) {
...
(function () {
    var w2form = function(options) {
        ...
        this.builder     = 0;        // can be null or 0
        this.fields    = [];
        this.actions   = {};
        this.record    = {};
        this.original  = {};
    ...
    w2form.prototype = {
        ...
        request: function (postData, callBack) { // if (1) param then it is call back if (2) then postData and callBack
            ...
            if (this.builder == null || typeof this.builder == 'undefined') this.builder = 0;
            // build parameters list
            var params = {};
            // add list params
            params['cmd']   = 'get-record';
            params['builder'] = this.builder;
            // append other params
            $.extend(params, this.postData);
        ...
        save: function (postData, callBack) {
            ...
            setTimeout(function () {
                ...
                if (w2utils.settings.dataType == 'RESTFULL') {
                    if (obj.builder != 0) ajaxOptions.type = 'PUT';
                    ajaxOptions.data = String($.param(ajaxOptions.data, false)).replace(/%5B/g, '[').replace(/%5D/g, ']');
                }
                if (w2utils.settings.dataType == 'JSON') {
                    ajaxOptions.type        = 'POST';
